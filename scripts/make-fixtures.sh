...
skip_if_exists "$F" || ffmpeg -v error -y "${VIDEO[@]}" "${TONE_A[@]}" -i "$WORK/ep.srt" \
  ...
  -c:a ac3 -b:a 192k -c:s srt -metadata:s:s:0 language=eng -shortest "$F" && echo "made  $(basename "$F")"
...
skip_if_exists "$F" || ffmpeg -v error -y "${VIDEO[@]}" "${TONE_A[@]}" "${TONE_B[@]}" -i "$WORK/anime.ass" \
  ...
  -c:s ass -metadata:s:s:0 language=eng -shortest "$F" && echo "made  $(basename "$F")"
