...
## 2026-09-05 — A remux seek beyond what is written restarts the remux

On the box, a 1 h 30 MKV remuxed to HLS showed a 10-minute bar that kept growing, a click into the middle stalled, and after a refresh the resume point was that click, so the page seeked into a timeline that starts at zero and never played. Remuxed segments follow the source keyframes, so the transcode trick (fixed segment boundaries, one shared timeline) does not apply. Instead the web player keeps the session's start offset: the bar shows the file's real length from ffprobe, positions and progress reports add the offset back, and `planSeek` decides whether a seek stays inside the written part or opens a new remux session at the target (the server already honours `startPositionMs` with `-ss`). The replaced session is deleted straight away rather than left to the idle sweep. Audio-track switches mid-file were broken the same way and use the same path now.

## 2026-09-05 — 10-bit and HDR video are decided on the stream, not the browser's word

The first 4K title on the box (HEVC Main 10, HDR10, in MKV) failed as a remux: Chrome answers "probably" for `hvc1.1.6` (8-bit HEVC) and that was taken as HEVC support in general. ffprobe now records each video stream's bit depth (from `pix_fmt`) and whether its transfer function is PQ or HLG, and existing libraries are backfilled from the raw probe JSON on startup, so no rescan. The decision copies 10-bit video only for profiles that list `hevc10` (the web probes `hvc1.2.4` separately; Android always lists it), and HDR only for profiles that declare `hdr` (Android does; browsers do not). Everything else is transcoded, with `tonemap_vaapi` to BT.709 on the GPU (or a zscale/tonemap chain in software) so colours come out right on an SDR screen. The player also now releases its HLS session on `pagehide` with a keepalive request, because retries after a failure had piled four remuxes of the same 4K file onto the process cap.
