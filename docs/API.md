...
| `POST /api/libraries/{id}/scan`  | admin                 | Walk the library now, reconcile files, and probe new or changed ones with ffprobe. Blocks until done and returns `ScanStatus` with `filesSeen`, `filesChanged`, `filesMissing`, `filesProbed`, `filesFailed`, `itemsLinked`. Becomes a background job in 1.10.                                                                       |
...
- **Items (1.9, 1.17)** — `FixMatchRequest`, `ProgressState` (always null on `ItemSummary.progress` until 1.17).
