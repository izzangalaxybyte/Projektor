...
/** Live on the TV with the remote: open a channel, Up changes channel, digits jump, Right opens the guide, catch-up plays. */
...
class TvLiveTest {
    ...
    private fun positionMs(): Long = textOf("position-ms").toLongOrNull() ?: 0L

    @Test
    fun opensAChannelChangesItAndPlaysCatchUp() {
        ...
        // Focusing a tab selects it; the first channel then takes focus itself.
        ...
        rule.waitUntil(40_000) { positionMs() > 1_000 }
        key(KeyEvent.KEYCODE_DPAD_UP)
        rule.waitUntil(15_000) { textOf("channel-name").contains("News 24") }
        key(KeyEvent.KEYCODE_1)
        rule.waitUntil(15_000) { textOf("channel-name").contains("Sport One HD") }

        key(KeyEvent.KEYCODE_DPAD_RIGHT)
        waitForTag("guide-panel")
        waitForTag("catchup-play")
        // The Earlier Match row is the first (and only) playable one and takes focus on open.
        Thread.sleep(400)
        key(KeyEvent.KEYCODE_DPAD_CENTER)
        waitForTag("position-ms")
        rule.waitUntil(40_000) { positionMs() > 1_000 }
        key(KeyEvent.KEYCODE_DPAD_DOWN) // show the bar
        waitForTag("catchup-title")
